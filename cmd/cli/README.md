...
```bash
./openagent keyring set my_provider_api_key sk-xxx
./openagent keyring set my_provider_base_url https://api.example.com/v1
./openagent keyring set my_provider_models model-a,model-b
```
...
| Command injection | `cli:commands` | Adds new `openagent` commands (e.g. `openagent stats`) |
