...
type AgentRunner interface {
	RunWithPrefix(ctx context.Context, session openagent.Session, prefix []openagent.Message, input openagent.Message) (*openagent.RunResult, error)
	RunStreamWithPrefix(ctx context.Context, session openagent.Session, prefix []openagent.Message, input openagent.Message) <-chan openagent.StreamEvent
}

// AgentBinder produces an AgentRunner for an in-process agent given the
// per-run team context. The application supplies it (e.g. kernel.New(cfg,
// deps) with handoff tools injected); Team calls it on every agent turn so
// per-run state (handoff tools, team prompt) is fresh.
type AgentBinder func(tc TeamContext) (AgentRunner, error)
...
type TeamContext struct {
	...
	HandoffTools   []openagent.Tool             // transfer_to_* tools to inject
}
...
type agentEntry struct {
	runner      AgentRunner // external/static runner (nil for binder-backed agents)
	binder      AgentBinder // in-process agents: per-run runner construction
	agentType   AgentType
	description string
}
...
// when to hand off.
func WithTeamAgent(name, description string, cfg *Agent, binder AgentBinder) TeamOption {
	return func(t *Team) {
		t.agents[name] = &agentEntry{binder: binder, agentType: AgentInternal, description: description}
...
// WithTeamObserver sets a openagent.RunObserver that receives stage events during team
// execution (agent starts, handoffs, etc.).
func WithTeamObserver(o openagent.RunObserver) TeamOption {
...
func (t *Team) AddAgent(name, description string, runner AgentRunner) error {
	...
	t.agents[name] = &agentEntry{runner: runner, agentType: AgentExternal, description: description}
	t.order = append(t.order, name)
...
// AddBinderAgent adds an in-process agent to the team at runtime via a
// per-run binder (which constructs the AgentRunner from TeamContext).
// Safe to call concurrently with Run / RunStream.
func (t *Team) AddBinderAgent(name, description string, binder AgentBinder) error {
	t.mu.Lock()
	if _, ok := t.agents[name]; ok {
		t.mu.Unlock()
		return fmt.Errorf("team: agent %q already exists", name)
...
func (t *Team) Run(ctx context.Context, session openagent.Session, input openagent.Message) (*TeamResult, error) {
	if len(t.agents) == 0 {
...
func (t *Team) RunStream(ctx context.Context, session openagent.Session, input openagent.Message) <-chan TeamEvent {
...
type teamRunner struct {
	...
	chain       []HandoffEntry
	runMessages []openagent.Message // all messages from this team run (used as prefix for next agent)
	totalTurns  int
...
func (tr *teamRunner) run(ctx context.Context, session openagent.Session, input openagent.Message, ch chan<- TeamEvent) (*TeamResult, error) {
	...
	for {
		...
		if len(tr.runMessages) > 128 {
			...
			for len(tr.runMessages) > 0 && tr.runMessages[0].Role == openagent.RoleTool {
			...
			for len(tr.runMessages) > 0 && tr.runMessages[0].Role == openagent.RoleAssistant && len(tr.runMessages[0].ToolCalls) > 0 {
				tr.runMessages = tr.runMessages[1:]
		...
		if vetoErr := tr.team.router.CanHandoff(ctx, entry, tr.chain, session); vetoErr != nil {
			...
			tr.forceFinal = true
			currentInput = openagent.Message{
				Transient: true,
...
func (tr *teamRunner) runAgentStreaming(ctx context.Context, runner AgentRunner, session openagent.Session, input openagent.Message, ch chan<- TeamEvent) (*openagent.RunResult, error) {
...
func (tr *teamRunner) prepareRunner(name string) AgentRunner {
	...
	if entry.binder != nil {
		runner, err := entry.binder(tr.buildTeamContext(name))
		if err != nil {
			return nil
		}
		return runner
...
func (tr *teamRunner) buildTeamContext(myName string) TeamContext {
	...
	var handoffTools []openagent.Tool
	if !tr.forceFinal {
		handoffTools = make([]openagent.Tool, 0, len(members))
...
// handoffTool is a openagent.Tool implementation for transfer_to_<name>.
...
func (t *handoffTool) Definition() openagent.FunctionDefinition {
	return openagent.FunctionDefinition{
		...
		EndTurn:     true,
		Parameters:  openagent.SchemaOf[HandoffParams](),
	}
}

func (t *handoffTool) Execute(_ context.Context, args json.RawMessage) *openagent.ToolResult {
	var params struct {
		Message string `json:"message"`
	}
	if err := json.Unmarshal(args, &params); err != nil {
		return openagent.ErrorResult(fmt.Errorf("transfer_to_%s: %w", t.target, err), false, "")
	}

	t.record(t.target, params.Message)
	return &openagent.ToolResult{Content: fmt.Sprintf("Transferred to %s.", t.target)}
}
...
type HandoffParams struct {
	Message string `json:"message" jsonschema:"description=What you need from the other agent. Be specific."`
}
